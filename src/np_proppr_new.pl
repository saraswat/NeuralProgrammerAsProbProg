/*
  ...
  The SLP/PCC model does not really work. 

  We introduce a new construct -- a conditional probability distribution

      X | V1, ..., Vk ~ pdspec

  One form of pdspec is X\Goal, for Goal a term. The intent is: execute bag(X, Goal, Xs), yielding in Xs the set of X's for which Goal is true. Assume this terminates. It is now required that the probability distribution from which X is to be sampled (and that will be learnt via training) will yield only values in Xs.

  (Note: Goal may have other free variables. TODO: Figure out if there needs to be any connection between the free varaibles and V1, ..., Vk.)

  Further, the user may wish to specify some information about how the pd is to be learnt. One technique is to 
  
  
  */
program(Input, Table, Ans) :-
